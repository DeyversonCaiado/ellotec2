...
export class CidadeList implements OnInit, AfterViewInit, OnDestroy {
  ...
  private buscarDoInicio(): void {
    ...
    this.service.listarPagina(this.pagina, PER_PAGE, this.termoBusca()).subscribe((resposta) => {
      this.cidades.set(resposta.items);
  ...
  carregarMais(): void {
    ...
    this.service.listarPagina(proximaPagina, PER_PAGE, this.termoBusca()).subscribe((resposta) => {
      ...
      this.cidades.update((lista) => [...lista, ...resposta.items]);
      this.temMais.set(resposta.items.length > 0 && resposta.page * resposta.perPage < resposta.total);
      this.carregandoMais.set(false);
    });
  }

  apagar(cidade: Cidade): void {
    if (!confirm(`Apagar a cidade "${cidade.nome}"? Essa ação não pode ser desfeita.`)) return;
    this.service.apagar(cidade.id).subscribe(() => {
      this.cidades.update((lista) => lista.filter((c) => c.id !== cidade.id));
    });
  }
}
