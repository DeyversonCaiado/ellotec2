...
def montar_payload(row):
    """Monta o payload do POST/PUT /estoque/saldos a partir de uma linha do banco.

    sistemaOrigemId é sintético (empresa+produto): FAT_ESTOQUE não tem uma
    chave própria, a linha é o par empresa+produto (mesma chave natural que a
    API usa no PUT sem id em /estoque/saldos). Quantidade negativa é levada a
    zero porque o schema da API não aceita valor negativo (visto também na
    integração Pharmanexo, que lida com o mesmo saldo)."""
    ...
    return {
        ...
        "sistemaOrigemId": f"{row['empresa']}-{row['codigo_pro']}",
...
def sincronizar_estoque_saldos():
    """Busca o saldo de produtos alterado e envia para o POST/PUT /estoque/saldos da API.
    ...
    Precisa rodar depois de produtos (a API rejeita o saldo se o produto
    ...
    with conectar() as connection:
        ...
        try:
            ...
            query = """
                ...
                WHERE
                    (:maior_data IS NULL OR e.DATA_HORA_ALTERACAO > :maior_data)
                ORDER BY
                    e.DATA_HORA_ALTERACAO
            """
    ...
    if len(rows) == 0:
        logger.info("Nenhum saldo de estoque novo/alterado para enviar.")
        return

    logger.info(f"Encontrados {len(rows)} saldos de estoque para enviar.")
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        if response.status_code in [200, 201]:
            logger.info(f"[{idx}/{len(rows)}] Saldo '{sistema_origem_id}' enviado com sucesso.")
        elif response.status_code == 409:
            logger.warning(
                f"[{idx}/{len(rows)}] Saldo '{sistema_origem_id}' já existe (409). "
                f"Atualizando pela chave natural (empresa+produto)..."
            )
            ...
            if update_response.status_code == 200:
                logger.info(
                    f"[{idx}/{len(rows)}] Saldo '{sistema_origem_id}' atualizado com sucesso."
                )
            else:
                raise RuntimeError(
                    f"Falha ao atualizar saldo '{sistema_origem_id}': "
                    f"{update_response.status_code} {update_response.text}"
                )
        else:
            raise RuntimeError(
                f"Falha ao enviar saldo '{sistema_origem_id}': "
                f"{response.status_code} {response.text}"
            )

        salvar_ultima_data(row["data_hora_alteracao"])

    logger.info("Todos os saldos de estoque do lote foram processados com sucesso.")


if __name__ == '__main__':
    sincronizar_estoque_saldos()
