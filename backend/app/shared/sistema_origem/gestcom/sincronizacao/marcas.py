...
def montar_payload(row):
    """Monta o payload do POST/PUT /marcas a partir de uma linha do banco."""
    return {
...
def sincronizar_marcas():
    """Busca marcas alteradas e envia para o POST /marcas da API.
        ...
        try:
            ...
            query = """
                SELECT
                    ...
                    fm.DATA_HORA_ALTERACAO
                FROM
                    fat_marcas fm
                WHERE
                    (:maior_data IS NULL OR fm.DATA_HORA_ALTERACAO > :maior_data)
                ORDER BY
                    fm.DATA_HORA_ALTERACAO
            """
    ...
    if len(rows) == 0:
        logger.info("Nenhuma marca nova/alterada para enviar.")
        return

    logger.info(f"Encontradas {len(rows)} marcas para enviar.")
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        if response.status_code in [200, 201]:
            ...
            if update_response.status_code == 200:
                logger.info(
                    f"[{idx}/{len(rows)}] Marca '{nome}' atualizada com sucesso."
                )
            else:
                raise RuntimeError(
                    f"Falha ao atualizar marca '{nome}' (sistema_origem_id={sistema_origem_id}): "
                    f"{update_response.status_code} {update_response.text}"
                )
        else:
            raise RuntimeError(
                f"Falha ao enviar marca '{nome}' (sistema_origem_id={payload['sistemaOrigemId']}): "
                f"{response.status_code} {response.text}"
            )

        salvar_ultima_data(row["data_hora_alteracao"])

    logger.info("Todas as marcas do lote foram processadas com sucesso.")
